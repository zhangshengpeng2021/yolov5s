...
def autopad(k, p=None):  # kernel, padding
    ...
    #   p: 自动计算的需要pad值（0填充）
    if p is None:
        #   isinstance(k, int)判断k为整数
        p = k // 2 if isinstance(k, int) else [x // 2 for x in k]  # auto-pad
    return p


def DWConv(c1, c2, k=1, s=1, act=True):
    #   深度可分离卷积，主要是将通道按输入输出的最大公约数进行切分，在不同的通道图层上进行特征学习
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        #   act: 激活函数类型   True就是SiLU()/Swish   False就是不使用激活函数
...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, c, num_heads):
        ...
        #   embed_dim每个头输入的维度×头的数量   num_heads即为注意头的总数量
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        #   expansion ratio c2*e=中间其他所有层的卷积核个数/中间所有层的输入输出channel数
...
class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions    简化版的BottleneckCSP
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        #   c1: 整个BottleneckCSP的输入channel
        #   c2: 整个BottleneckCSP的输出channel
        #   n: 有n个Bottleneck
...
#   用在了模型的一开始，将输入图像先 slice 成4份，再做concat
...
class Expand(nn.Module):
    ...
    #   将feature map的w和h维度(放大)的数据收缩到channel维度上(缩小)
    def __init__(self, gain=2):
        super().__init__()
        self.gain = gain
...
#   model封装成包含前处理、推理、后处理的模块(预处理 + 推理 + nms)
class autoShape(nn.Module):
    # input-robust model wrapper for passing cv2/np/PIL/torch inputs. Includes preprocessing, inference and NMS
    #   在train中不会被调用，当模型训练结束后，会通过这个模块对图片进行重塑，来方便模型的预测自动调整shape
